...
#include "core/cheats/cheats_data.h"
...
void GatewayCheat::SetBuiltIn(bool builtIn) {
    built_in = builtIn;
}

bool GatewayCheat::IsBuiltIn() const {
    return built_in;
}

// based on cheats in https://github.com/Slade1972/CTRPF-AR-CHEAT-CODES
// This now parses the byte array generated by our CMake Python script
static nlohmann::json cheats_json =
    nlohmann::json::parse(reinterpret_cast<const char*>(cheats_json_data),
                          reinterpret_cast<const char*>(cheats_json_data + cheats_json_data_size));
...
std::vector<std::shared_ptr<CheatBase>> GatewayCheat::LoadFile(const std::string& filepath) {
    ...
    std::unordered_map<std::string, bool> cheatNames;
    std::string titleId = "";
    std::string comments;
    ...
    if (filepath.length() > 20) {
        titleId = filepath.substr(filepath.length() - 20, 16);
    }
            ...
            while (!file.eof()) {
                ...
                if (line.length() >= 2 && line.front() == '[') {
                    if (!cheat_lines.empty()) {
                        ...
                        cheats.push_back(
                            std::make_shared<GatewayCheat>(name, cheat_lines, comments));
                        cheats.back()->SetEnabled(enabled);
                        cheats.back()->SetBuiltIn(built_in);
                    }
                    enabled = false;
                    built_in = false;
